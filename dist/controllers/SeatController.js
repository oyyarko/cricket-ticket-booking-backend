  ...
  try {
    ...
    for (const seatSelection of selectedSeats) {
      if (!Array.isArray(seatSelection) || seatSelection.length !== 2) {
        return res.status(400).json({
          message: "Invalid seat selection format: Each seat selection must be an array with two elements [row, col]"
        });
      }
      const row = seatSelection[0];
      const col = seatSelection[1];

      await SeatModel.findOneAndUpdate({ row, col, block: blockId }, { status: 1 });
